...
def main():
    ...
    gr_t        = '/media/data_4T/margaryta/base/patient_level/swin/upernet_swin_tiny_patch4_160k_wmh_ce/pred' # For example: '/output/teamname/0'
    ...
    for patient in patients:
        ...
        res_dict = computeQualityMeasures(array_pred, array_gr_t, (1,2,3), ['dice', 'jaccard', 'precision', 'recall', 'fpr', 'fnr', 'vs', 'hd', 'hd95', 'mdsd', 'stdsd'])
        dices.append(res_dict.get('dice', None))
        jaccard.append(res_dict.get('jaccard', None))
        precision.append(res_dict.get('precision', None))
        recall.append(res_dict.get('recall', None))
        fnr.append(res_dict.get('fnr', None))
        fpr.append(res_dict.get('fpr', None))
        vs.append(res_dict.get('vs', None))
        mdsd.append(res_dict.get('mdsd', None))
        stdsd.append(res_dict.get('stdsd', None))
        hd95.append(res_dict.get('hd95', None))

        
    #Creating subplot of each column with its own scale
    red_circle = dict(markerfacecolor='red', marker='o', markeredgecolor='white')
    data = [dices, jaccard, precision, recall, fnr, fpr, vs, mdsd, stdsd, hd95]
    titles = ['Dice', 'Jaccard', 'Precision', 'Recall', 'False positive rate', 'False negtive rate', 'Volume similarity', 'Hausdorff distance \n 95% percentile', 'Median surface distance', 'Std surface distance']
    ...
    fig.suptitle('Swin Transformer with Cross Entropy loss', size=30)
    plt.tight_layout()
    '''
    ...
    ax.boxplot(data)
    '''
    plt.savefig('/media/data_4T/margaryta/base/patient_level/swin/upernet_swin_tiny_patch4_160k_wmh_ce/pred/boxplots.png')
